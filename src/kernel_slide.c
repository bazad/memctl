...
kernel_slide_init_ios_unsafe() {
	...
	// Scan for the kernel base.
	for (kword_t slide = 0; slide < max_slide; slide += slide_increment) {
		// TODO: This should be based on kernelcache's base address.
		kaddr_t base = region_base + slide + 0x4000;
		kword_t value;
		kernel_io_result kior = kernel_read_word(kernel_read_unsafe, base, &value,
				sizeof(value), 0);
		if (kior != KERNEL_IO_SUCCESS) {
			continue;
		}
		if (value != 0x0100000cfeedfacf) {
			continue;
		}
